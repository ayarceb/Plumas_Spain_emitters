...
async function loadEmissionSeriesForSites(sites, statusEl) {
    ...
    entries.forEach(entry => {
        if (entry.series && Array.isArray(entry.series.values)) {
            ...
            const mean = Number.isFinite(entry.series.mean) ? entry.series.mean : null;
            result.set(entry.id, { ...entry.series, normalized, globalMin, globalMax, mean });
            found++;
        }
    });
...
async function main() {
    ...
    emissionSeriesPromise.then(seriesMap => {
        ...
        // Reemplaza el valor anual provisional (15 kt/año) por la media real
        // de cada serie cargada, de modo que las etiquetas y los fallbacks
        // reflejen el valor específico de cada planta.
        if (seriesMap && plantsGeoJSON) {
            plantsGeoJSON.features.forEach(feature => {
                const series = seriesMap.get(feature.id);
                if (series && Number.isFinite(series.mean)) {
                    feature.properties.annual = series.mean;
                }
            });
            const plantsSource = map.getSource("plants");
            if (plantsSource) {
                plantsSource.setData(plantsGeoJSON);
            }
        }
    });
